...
`timescale 1ns / 1ps
`include "versat_ai_uart_csrs.vh"
`include "versat_ai_uart_conf.vh"

module versat_ai_uart_core (
   input                            clk_i,
   input                            arst_i,
   input                            rst_soft_i,
   input                            tx_en_i,
   ...
   input      [`VERSAT_AI_UART_CSRS_DIV_W-1:0] bit_duration_i
);
   ...
   localparam RST_POL = `VERSAT_AI_UART_RST_POL;
      ...
      if (RST_POL == 1) begin : g_rst_pol_1
         ...
         always @(posedge clk_i, posedge arst_i) begin
            if (arst_i) begin
               ...
               fifo_wen_o  <= 1'b0;
               rs232_rts_o <= 1'b0;
            end else if (~fifo_full_i && rx_en_i) begin
