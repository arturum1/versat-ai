...
// TX FUNCTIONS
void uart_txwait() {
  while (!versat_ai_uart_csrs_get_txready())
    ;
}
...
void uart_putc(char c) {
  while (!versat_ai_uart_csrs_get_txready())
    ;
  versat_ai_uart_csrs_set_txdata(c);
}
...
uint8_t uart_getc() {
  while (!versat_ai_uart_csrs_get_rxready())
    ;
  return versat_ai_uart_csrs_get_rxdata();
}

// UART basic functions
void uart_init(int base_address, uint16_t div) {
  // capture base address for good
  versat_ai_uart_csrs_init_baseaddr(base_address);

  // pulse soft reset
  versat_ai_uart_csrs_set_softreset(1);
  versat_ai_uart_csrs_set_softreset(0);
  ...
  // versat_ai_uart_csrs_set_div(868)
  versat_ai_uart_csrs_set_div(div);

  // enable TX and RX
  versat_ai_uart_csrs_set_txen(1);
  versat_ai_uart_csrs_set_rxen(1);
}
