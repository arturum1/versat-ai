...
//        assert(!compressedGen || cmdToRspStageCount == 1, "Can't combine DYNAMIC_TARGET and RVC as it could stop the instruction fetch mid-air")
        ...
        val history = Mem(BranchPredictorLine(), 1 << historyRamSizeLog2) init {
          // Create a default initialized BranchPredictorLine
          val defaultLine = BranchPredictorLine()
          defaultLine.source := B(0, 30 - historyRamSizeLog2 bits)
          defaultLine.branchWish := U(0, 2 bits)
          defaultLine.target := U(0, 32 bits)
          if(compressedGen) defaultLine.last2Bytes := False
          
          // Initialize all entries to the default value
          for (i <- 0 until (1 << historyRamSizeLog2)) yield {
            defaultLine
          }
        }
